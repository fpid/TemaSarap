<article id="{$id}" class="entry {$date|date_format:"y-%Y m-%m d-%d"}">
	<header class="entry-header page-header">
		<h3 class="entry-title"><a href="{$id|link:comments_link}">{$subject|tag:the_title}</a></h3>
				...
				<i class="fa fa-calendar"></i><a href="{$smarty.const.BLOG_BASEURL}?x=entry:{$id}" rel="bookmark">{$date|date_format_daily:"`$fp_config.locale.dateformat`"}</a>    				<time class="entry-date published" datetime="{$date|date_format_daily:"`$fp_config.locale.dateformat`"}"><i class="fa fa-user"></i><a href="https://plus.google.com/100167480261171437157?rel=author"  rel="publisher" title="ßägüž Äch ( {$author} )"> {$author}</a>
